...
// @dev The Sensei contract is a proxy contract that forwards administrative function calls to FarmMaster.
// @dev Sensei is the owner of the FarmMaster and as such has access to all owner delegated functions.
// @dev The Admin/Dev team will own Sensei and can make calls to FarmMaster owner functions via Sensei.
// @dev In order to overcome Migrator vunerabilities within the existing FarmMaster contract Sensei
// @dev will NOT forward calls to the migrator functions in FarmMaster thus making these functions 
// @dev inaccessible to all users. 
// @dev In addition Sensei will not forward calls to change the ownership of the FarmMaster thus making 
// @dev Sensei the permenant gardian of the FarmMaster.  The ownership of Sensei is transferable
...
contract Sensei is Ownable {
 ...
 // @dev Set the forwarding address to the currently deployed FarmMaster.
 // @dev Ownership of the FarmMaster must be transferred to this contract once this contract is deployed
    constructor(address ZenAddress, address[] memory initialDelegates) public { 
        Zen = FarmMaster(ZenAddress);
...
// @dev The functions below are the functions that will be relayed to the FarmMaster
// @dev Any other functions in the FarmMaster that are 'onlyOwner' will become innaccessible
