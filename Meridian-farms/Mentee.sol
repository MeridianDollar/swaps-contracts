...
contract Mentee {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many xCHARM tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of xCHARMs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws xCHARM tokens to a pool. Here's what happens:
    ...
    // The xCHARM TOKEN!
    IBEP20 public xcharm;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes xCharm tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = xcharm.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 xcharmSupply = xcharm.balanceOf(address(this));
        if (xcharmSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(xcharmSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit xCharm tokens to Mentee for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        xcharm.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw xCharm tokens from Mentee.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        xcharm.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        xcharm.safeTransfer(address(msg.sender), user.amount);
